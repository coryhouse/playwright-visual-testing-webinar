...
import Logo from "../../../E-Commerce/components/Header/logo";
...
export default function LoginForm(
    ...
    const updateSize = useCallback(() => {
        ...
        formContainer.style.width = formContainer.parentElement.offsetWidth / 2 + 'px';
    }, [])
    ...
    const handleSignIn = () => {
        if (restrictSignIn && (username?.length == 0 || password?.length == 0 || (extraInput && (confirmPassword?.length == 0 || password != confirmPassword)))) {
            setShowError(true);
            ...
            } else if (extraInput && (confirmPassword?.length == 0 || password != confirmPassword)) {
    ...
    return (
        <div style={theme ? theme1.loginScreen : theme2.loginScreen} className={loginScreen}>
            {showBanner && (
                <h1 style={theme ? theme1.banner : theme2.banner} className={banner}>
                    <span style={{display: 'inline-flex', flexDirection: 'column', justifyContent: 'center', height: headerHeight}}>Welcome, please sign in below</span>
                </h1>
            )}
			{rearrange ? (
				<div className={loginFormContainer}>
					<div style={theme ? {...theme1.loginForm, display: 'flex', justifyContent: 'space-evenly', maxWidth: '1000px'} : {...theme2.loginForm, display: 'flex', maxWidth: '1000px', justifyContent: 'space-evenly', padding: '100px 0px'}} className={loginForm}>
						<div>
							<div className={loginFormLogo} style={{textAlign, maxHeight: '200px'}}>
								{theme ? 
									<a href="/">
										<img alt="" src={logo.src} style={{display: "inline-block", height: '50px'}} data-id="login-logo"/>
									</a>
									:   
									<a href="/">
							...
							<h4 style={theme ? {...theme1.loginFormHeader, fontSize: '35px', border: 'none'} : {...theme2.loginFormHeader, fontSize: '35px', border: 'none', textAlign: 'center'}}>
						...
						<div>
							<form>
								...
								<div className={loginFormGroup}>
									...
									/>
									<div style={theme ? theme1.loginFormInputIcon : theme2.loginFormInputIcon} className={[loginFormInputIcon, maleCircleIcon].join(" ")}></div>
								</div>
								...
								<div className={loginFormGroup}>
									...
									/>
									<div style={theme ? theme1.loginFormInputIcon : theme2.loginFormInputIcon} className={[loginFormInputIcon, fingerprintIcon].join(" ")}></div>
								</div>
								{extraInput && (
									<div className={loginFormGroup}>
										<label>Confirm Password</label>
										<input
											id={changeId ? 'password-field-confirm' : 'password-confirm'}
											placeholder="Confirm your password" 
											type="password"
											onChange={(e) => handleUpdate('passConfirm', e.target.value)}
										/>
										<div style={theme ? theme1.loginFormInputIcon : theme2.loginFormInputIcon} className={[loginFormInputIcon].join(" ")}>
								...
								<div className={formOptionsContainer}>
									<button style={theme ? theme1.formSubmit : theme2.formSubmit} className={formSubmit} type="button" onClick={handleSignIn}>
				...
				<div className={loginFormContainer}>
					<div style={theme ? theme1.loginForm : theme2.loginForm} className={loginForm}>
						<div className={loginFormLogo} style={{textAlign, maxHeight: '200px'}}>
							{theme ? 
								<a href="/">
									<img alt="" src={logo.src} style={{display: "inline-block", height: '50px'}} data-id="login-logo"/>
						...
						<h4 style={theme ? theme1.loginFormHeader : theme2.loginFormHeader} className={loginFormHeader}>
							Login Form
							<div style={theme ? theme1.borderAccent : theme2.borderAccent}></div>
						</h4>
						{showError && (
